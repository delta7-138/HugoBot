...
class Shoegaze(commands.Cog):
    ...
    async def getShoegazedImage(self , imgurl , urlFlag , color):
        flag = False
    ...
    async def shoegazeavatar(self , ctx , member:discord.Member , color):
        output = await self.getShoegazedImage(member.avatar_url , 0 , color) 
        if(output!=None):
    ...
    async def shoegazecolor(self , ctx , color):
        output = await self.getShoegazedImage(ctx.message.author.avatar_url , 0 , color)
        if(output!=None):
            await ctx.send(file = output[0] , embed = output[1])
        else:
            await ctx.send("invalid color input")
    ...
    async def shoegazecolorerr(self , ctx , err):
        if isinstance(err , commands.MissingRequiredArgument):
            output = await self.getShoegazedImage(ctx.message.author.avatar_url , 0 , "p")
            if(output!=None):
                await ctx.send(file = output[0] , embed = output[1])
            else:
                await ctx.send("Invalid color input")
    ...
    async def shoegazeimage(self , ctx , url , color):
        output = await self.getShoegazedImage(url ,0 ,  color)
        if(output!=None):
            await ctx.send(file = output[0] , embed = output[1])
        else:
            await ctx.send("Invalid color input")
    ...
    async def shoegazegif(self , ctx , url):
        frames = []
        ...
        for i in range(im.n_frames):
            try:
                im.seek(i)
                # im.save('tmp.png')
                # output = await self.getShoegazedImage('tmp.png' , 1 , "p")
                # im = Image.open('res.jpg')
                tmp = await self.distortImage(im)
                frames.append(tmp)
            except: 
                break

        frames[0].save('res.gif' , format = 'GIF' , append_images = frames[1:] , save_all = True , duration = 300 , loop = 0)
        fil = discord.File('res.gif')
        embed = discord.Embed(title = "Distorted Gif" , color = 0xff00ff)
        embed.set_image(url = "attachment://res.gif")
